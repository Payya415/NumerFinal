...
let xm=xl;
let arrayOject = []
 export const BisectionMethod = (x,xl, xr) => {

const functionBisection =(eq,xm) =>{
  const equation = evaluateTex(eq,{x:xm})
  return equation.evaluated
}
if(functionBisection(x,xl)*functionBisection(x,xr)>=0){
  alert("A*B Must less than 0")
  return
}
  while(xr-xl>=EPSILON){
    xm = (xl+xr)/2
    if(functionBisection(x,xm)===0.0){
      break
    }else if(functionBisection(x,xm)*functionBisection(x,xl)<0){
      xr = xm
  ...
  // while (true){
  // xm = (xl + xr) / 2;
  //   if (functionX(xm) === 0.0) break;
  //   else if (functionX(xm) * functionX(xl) < 0) {
  //       xr = xm
  //       error = Math.abs((xr - xl)/xr).toFixed(6)
  //   }
  //   else {
